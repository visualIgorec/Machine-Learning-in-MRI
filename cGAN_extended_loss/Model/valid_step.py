...
def valid_step(valid_loader, BCE_criterion, Loss, gen, disc, device, common_list, alpha):
    
    for index_b, (feature, target) in enumerate(valid_loader):
        ...
        common_list[-2].append(G_Loss.item())
        common_list[-1].append(D_Loss.item())
    
    g_mean_loss = sum(common_list[-2])/len(common_list[-2])
    d_mean_loss = sum(common_list[-1])/len(common_list[-1])
    
    common_list[5].append(g_mean_loss)
    common_list[6].append(d_mean_loss)
